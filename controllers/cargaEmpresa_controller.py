...
from models.cargaEmpresa_model import CargaPorEmpresa

class CargaEmpresaController:

    def get_cargas_por_empresa(self, empresa_id: int):
        try:
            ...
            if not result:
                raise HTTPException(status_code=404, detail="No se encontraron cargas para la empresa especificada")
            
            cargas = []
            for data in result:
                carga = CargaPorEmpresa(
                    id=data[0],
                    peso=data[1],
                    largo=data[2],
                    ancho=data[3],
